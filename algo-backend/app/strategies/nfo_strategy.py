...
from app.execution.angel_execution import (
...
def check_signal(df):
    try:
        ...
        if close < OR_low and volume > 1.5 * avg_vol:
            sl = OR_mid
            target = entry_price - TARGET_POINTS
            return "SELL", {
                "sl": sl,
        ...
        logger.error(f"NFO Signal Error: {exc}")
        return None, None
...
def backtest(broker, selected_symbols=None, interval="FIVE_MINUTE", from_date="2022-01-01 09:15", to_date="2022-03-31 15:30", stock_list=None):
    ...
    for stock in target_stocks:
        ...
        data_response = fetch_historical_data(
            broker, exchange, symbol_token, interval, from_date, to_date
        )
                    ...
                    if position is None and trade_count < TRADE_LIMIT_DAILY:
                        ...
                        if row["Close"] > OR_high and row["Close"] > row["vwap"] and row["volume"] > 1.5 * avg_vol:
                            position = {
                                ...
                                "entry": row["Close"],
                                "sl": OR_mid,
                                "target": row["Close"] + TGT_PTS,
                                "qty": QTY,
                            }
                            logger.info(
                                f"[NFO TRADE] BUY {trading_symbol} @ {row['Close']} {idx.date()}"
                            )
                            ...
                            logger.info(
                                f"[NFO TRADE] SELL {trading_symbol} @ {row['Close']} {idx.date()}"
                            )
